...
/*
    ...
    As noted above, the arguments are dual-function. For example,
        ...
        used for logging.
*/
function Log(baseFileName, fileExt) {
    /*
...
/*
Disabled the creation of a timestamped log file name. This makes  
it easier during development while tailing the log file. However,
for production a timestamped (and managed) log file is preferred.

To Do: implement log file managment in regards to the size of the
log file. When it reaches a specified maximum size (see log-opt.js)
a new log file with a timestamped name will be created.

            // maybe, if we've got it then use it...
            if(baseFileName !== '') {
                this.file = this.file + baseFileName +'-';
            }
            // create the time stamp portion of the file name
            this.file = this.file + timeStamp(true);
*/
            // not using timestamped file name...
            this.file = this.file + baseFileName;
/**/
